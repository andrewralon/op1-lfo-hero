...
- [x] **A way for other people to measure their hardware for us.** `docs/probe/` — a guided
      browser probe plus a manual instruction path for anyone Web MIDI can't reach (Safari has
      never shipped it, on macOS or iOS). Both walk the same numbered steps from `steps.js` and
      land in the same issue template. Reports collect in `notes/DEVICE_REPORTS/`
- [ ] **Get an OP-1 OG report.** The whole point — it is the one device people most often ask
      about and the one we can't test
...
- [ ] **Detection fallback on manufacturer/model.** CoreMIDI exposes `teenage engineering` and
      ...
      that rename ports. **Blocked on data, not on code** — the device-report probe
      (`docs/probe/`) records every port's `name` / `manufacturer` / `version` verbatim, so the
      first few submissions will show whether the fallback is worth building and what strings to
      match on
