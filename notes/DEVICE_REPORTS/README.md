...
report they came from. Profile literals go in `ios/Sources/Engine/DeviceProfiles.swift`.
...
Also check `channel` and `cc` **independently** across a device's tracks. The app can currently only
express "same CC, channel varies per track" (`ParamBinding.cc` takes a fixed `cc` and a
`ChannelRule`). A device that instead varies the CC on one channel needs a `ChannelRule`-equivalent
for the CC number before it can be supported at all.
