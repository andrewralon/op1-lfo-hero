...
`analysis.primary` on each capture is the mapper's *guess*, not a fact. Check it against the raw
...
- `ambiguous: true` — the mapper genuinely can't tell two encodings apart from what it saw. The
...
only one of the two cannot be read back later. The mapper captures both — keep it that way.
