...
- [x] Remote control web UI: a mobile-friendly control surface served at `http://<mac-ip>:8765/` over local WiFi. The desktop Python engine (`automation.py`, `clock.py`, `controller.py`) runs unchanged; a FastAPI + uvicorn WebSocket server (`src/server.py`) exposes it to any browser on the same network. Controls: BPM, transport (play/stop/tape nav), per-track volume/pan/mute, LFO start/stop with full wave+rate+depth+center+param+track selection. State broadcast every 200 ms; auto-reconnects on disconnect. Works in Mobile Safari — no app install required.
