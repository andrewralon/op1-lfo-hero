...
This is a *first draft*, not an answer. Every inference the probe was unsure about is
...
(see notes/RESEARCH.md for the six places the published TE references disagree with
the hardware). Read every warning before pasting anything into DeviceProfiles.swift.
"""
...
def track_addressing(caps, step_id):
    ...
    if len(chans) == 1 and len(ccs) > 1:
        block(
            ...
            "see notes/FIELD_DEVICE_SUPPORT.md."
        )
        return None
