...
reference has repeatedly produced wrong answers. `docs/probe/` is a crowdsourcing page published on
GitHub Pages (`https://andrewralon.github.io/op1-lfo-hero/probe/`) that walks someone who owns a
...
| `docs/probe/manual.html` | the same steps as instructions for an off-the-shelf MIDI monitor |
...
- **Pages builds from `main`.** The probe is unreachable from a branch. Test locally with
